  ...
  return (
    <div className="w-[250px] m-auto flex flex-col items-center justify-center h-screen">
      ...
      <span className="progress bg-gray-200 rounded w-full h-3 mb-4 relative">
        <p className="absolute -translate-y-4 right-0 top-0 text-xs text-indigo-800">{progress}%</p>
      ...
      {status == 'done' && <p className="text-indigo-900 text-xs">You can access the file at {res.secure_url}</p>}
    </div>
  );
};
